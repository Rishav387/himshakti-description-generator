...
// ── ProductForm OUTSIDE Dashboard — prevents remount on every keystroke ────
function ProductForm({ form, onChange }) {
  return (
    <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-1">

      <div>
        <label className="block text-sm font-semibold text-earth-800 dark:text-earth-100 mb-1.5">Product Name *</label>
        <input name="name" value={form.name} onChange={onChange}
          placeholder="e.g. Himalayan Millet Bar"
          className="w-full px-4 py-2.5 rounded-xl text-sm border border-earth-200 dark:border-earth-600 bg-earth-50 dark:bg-earth-700 text-earth-900 dark:text-earth-50 placeholder:text-earth-400 dark:placeholder:text-earth-500 focus:outline-none focus:ring-2 focus:ring-saffron-400 transition" />
      </div>

      <div>
        <label className="block text-sm font-semibold text-earth-800 dark:text-earth-100 mb-1.5">Category *</label>
        <select name="category" value={form.category} onChange={onChange}
          className="w-full px-4 py-2.5 rounded-xl text-sm border border-earth-200 dark:border-earth-600 bg-earth-50 dark:bg-earth-700 text-earth-900 dark:text-earth-50 focus:outline-none focus:ring-2 focus:ring-saffron-400">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-sm font-semibold text-earth-800 dark:text-earth-100 mb-1.5">Price (₹) *</label>
          <input name="price" type="number" value={form.price} onChange={onChange}
            placeholder="199"
            className="w-full px-4 py-2.5 rounded-xl text-sm border border-earth-200 dark:border-earth-600 bg-earth-50 dark:bg-earth-700 text-earth-900 dark:text-earth-50 placeholder:text-earth-400 dark:placeholder:text-earth-500 focus:outline-none focus:ring-2 focus:ring-saffron-400 transition" />
        </div>
        <div>
          <label className="block text-sm font-semibold text-earth-800 dark:text-earth-100 mb-1.5">Weight *</label>
      ...
      <div>
        <label className="block text-sm font-semibold text-earth-800 dark:text-earth-100 mb-1.5">Description *</label>
        <textarea name="description" value={form.description} onChange={onChange}
          placeholder="Brief product description" rows={2}
          className="w-full px-4 py-2.5 rounded-xl text-sm border border-earth-200 dark:border-earth-600 bg-earth-50 dark:bg-earth-700 text-earth-900 dark:text-earth-50 placeholder:text-earth-400 dark:placeholder:text-earth-500 focus:outline-none focus:ring-2 focus:ring-saffron-400 resize-none transition" />
      </div>

      <div>
        <label className="block text-sm font-semibold text-earth-800 dark:text-earth-100 mb-1.5">Ingredients *</label>
        <input name="ingredients" value={form.ingredients} onChange={onChange}
          placeholder="e.g. Millet, jaggery, ghee"
          className="w-full px-4 py-2.5 rounded-xl text-sm border border-earth-200 dark:border-earth-600 bg-earth-50 dark:bg-earth-700 text-earth-900 dark:text-earth-50 placeholder:text-earth-400 dark:placeholder:text-earth-500 focus:outline-none focus:ring-2 focus:ring-saffron-400 transition" />
      </div>

      <div>
        <label className="block text-sm font-semibold text-earth-800 dark:text-earth-100 mb-1.5">Features (comma separated)</label>
        <input name="features" value={form.features} onChange={onChange}
          placeholder="No Preservatives, Handmade"
          className="w-full px-4 py-2.5 rounded-xl text-sm border border-earth-200 dark:border-earth-600 bg-earth-50 dark:bg-earth-700 text-earth-900 dark:text-earth-50 placeholder:text-earth-400 dark:placeholder:text-earth-500 focus:outline-none focus:ring-2 focus:ring-saffron-400 transition" />
      </div>

      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-sm font-semibold text-earth-800 dark:text-earth-100 mb-1.5">Badge</label>
          <input name="badge" value={form.badge} onChange={onChange}
            placeholder="Bestseller"
            className="w-full px-4 py-2.5 rounded-xl text-sm border border-earth-200 dark:border-earth-600 bg-earth-50 dark:bg-earth-700 text-earth-900 dark:text-earth-50 placeholder:text-earth-400 dark:placeholder:text-earth-500 focus:outline-none focus:ring-2 focus:ring-saffron-400 transition" />
        </div>
        <div>
          <label className="block text-sm font-semibold text-earth-800 dark:text-earth-100 mb-1.5">Emoji</label>
          <input name="emoji" value={form.emoji} onChange={onChange}
            placeholder="🌿"
            className="w-full px-4 py-2.5 rounded-xl text-sm border border-earth-200 dark:border-earth-600 bg-earth-50 dark:bg-earth-700 text-earth-900 dark:text-earth-50 focus:outline-none focus:ring-2 focus:ring-saffron-400 transition" />
        </div>
      </div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <div className="bg-saffron-50 dark:bg-saffron-900/20 border border-saffron-100 dark:border-saffron-800 rounded-xl px-5 py-3 mb-6 flex items-center gap-3">
            ...
            <p className="text-sm text-saffron-700 dark:text-saffron-300">
              You are viewing and managing only <strong>your own products</strong>. Other users cannot modify your catalog.
            </p>
          </div>

          {loading ? (
            <div className="py-20"><Loader variant="spinner" size="lg" label="Loading your products…" /></div>
          ) : products.length === 0 ? (
            <div className="text-center py-20 bg-earth-50 dark:bg-earth-800 rounded-2xl border border-earth-100 dark:border-earth-700">
              ...
              <h3 className="text-xl text-earth-800 dark:text-earth-100 mb-2" style={{ fontFamily: "Georgia, serif" }}>
                No products yet
              </h3>
                        ...
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-earth-400 dark:text-earth-400">{p.weight}</p>
                            </div>
                          </div>
                        </td>
